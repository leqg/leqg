var sms = function() {
	...
	// Données relative au système d'export
		...
		$("#exportation").on('click', function() {
			...
			var destination = 'index.php?page=poste&action=historique&campagne=' + campagneId;
	...
	// Scripts relative au système de critère géographique de l'export
		...
		$('#ville-recherche').keyup(function(){
			...
			if (value.length >= 3) {
				$.ajax({
					type: 'POST',
					url: 'ajax.php?script=poste-export-ville',
		...
		$('#rue-recherche').keyup(function(){
			...
			if (value.length >= 3) {
				$.ajax({
					type: 'POST',
					url: 'ajax.php?script=poste-export-rue',
