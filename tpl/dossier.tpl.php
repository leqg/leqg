...
<section id="fiche">
	<header class="dossier">
		...
		<a class="nostyle" id="config-icon" href="<?php $core->tpl_go_to('dossier', array('id' => $dossier['id'], 'modifierInfos' => 'true')); ?>">&#xe855;</a>
	</header>
	
	<ul class="deuxColonnes">
		<li>
			<span class="label-information"><label for="description">Ouverture du dossier</label></span>
			<p><?php echo date('d / m / Y', strtotime($dossier['date_ouverture'])); ?></p>
		</li>
		<li>
			<span class="label-information"><label for="description">Description</label></span>
			<p><?php echo nl2br($dossier['description']); ?></p>
		</li>
		<li>
			<span class="label-information">Fichiers associés au dossier</span>
			<ul class="listeEncadree">
				<?php if ($fichier->nombreFichiers('dossier' , $dossier['id'])) : ?>
				<?php $fichiers = $fichier->listeFichiers('dossier' , $dossier['id']); foreach ($fichiers as $f) : ?>
				...
				<a href="<?php $core->tpl_go_to('dossier', array('id' => $dossier['id'], 'fichier' => 'true')); ?>">
...
<aside>
	<div id="historique">
		...
		<?php $interactions = $historique->rechercheParDossier($dossier['id']); // On initialise la liste des interactions à afficher ?>
	...
	<div id="modifierInfos">
		<nav class="navigationFiches">
			<a class="retour" href="<?php $core->tpl_go_to('dossier', array('id' => $dossier['id'])); ?>">Retour au dossier</a>
		</nav>
		...
		<form action="ajax.php?script=modification-dossier" method="post">
			...
			<ul class="deuxColonnes">
				<li>
					...
					<input type="text" name="nom" id="form-nom" value="<?php echo $dossier['nom']; ?>">
				</li>
				<li>
					<span class="label-information">Description</span>
					<textarea name="description" id="form-description"><?php echo $dossier['description']; ?></textarea>
				</li>
				<li class="submit">
