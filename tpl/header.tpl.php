...
<body class="flat">	
	...
	<header id="top">
		...
		<a class="nostyle" id="notifications" href="#" title="Afficher les notifications"><?php $nb_taches = Evenement::userTasks(); if ($nb_taches) : ?><span></span><?php 
 endif; ?></a>
		<a class="nostyle" id="rechercheRapide" href="#" title="Rechercher une fiche"></a>
		<form method="post" action="<?php Core::goPage('recherche'); ?>"><input type="search" id="searchForm" name="recherche" pattern=".{3,}" placeholder="Michel Dupont" autocomplete="off"><input type="submit" value="&#xe803;" id="searchSubmit"></form>
	</header><!--header#top-->
    ...
    if (User::auth_level() >= 5) :
       ...
       <a href="<?php Core::goPage($key); ?>" id="lien-<?php echo $key; ?>" <?php if ($actuel == $key) { echo 'class="actif"'; 
      } ?>><?php echo $element; ?></a>
        <?php endforeach; ?>
   <a href="http://auth.leqg.info/deconnexion.php" id="lien-logout">Déconnexion</a>
    <?php else: ?>
		<a href="<?php Core::goPage('porte', array('action' => 'missions')); ?>" id="lien-porte">Porte-à-porte</a>
		<a href="<?php Core::goPage('boite', array('action' => 'missions')); ?>" id="lien-boite">Boîtage</a>
		<a href="<?php Core::goPage('rappels', array('action' => 'appel')); ?>" id="lien-rappels">Rappels</a>
		<a href="http://auth.leqg.info/deconnexion.php" id="lien-logout">Déconnexion</a>
    <?php endif; ?>
	</nav><!--nav#principale-->
