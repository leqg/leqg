<?php
	...
	$query = 'UPDATE `users` SET `user_new_email` = "' . $donnees['email'] . '", `user_new_email_hash` = "' . $hash . '" WHERE `user_id` = ' . $donnees['user'];
	$noyau->query($query);
	...
	$email = strtr($email, array('{USER}' => $user->get_the_nickname()));
	$email = strtr($email, array('{URL}' => 'http://' . $config['SERVER']['url'] . '/validation-email.php?email=' . $hash));
