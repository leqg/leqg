...
if ($contact1['electeur'] == 1 || $contact2['electeur'] == 1) :
    ...
    if ($contact1['electeur'] == 1 && $contact2['electeur'] == 0) :
        ...
        if (isset($infos['email'])) : $query.= '`contact_email` = "' . $infos['email'] . '", '; else : $query.= '`contact_email` = NULL, '; 
        endif;
        if (isset($infos['fixe'])) : $query.= '`contact_telephone` = "' . $infos['fixe'] . '", '; else : $query.= '`contact_telephone` = NULL, '; 
        endif;
        if (isset($infos['mobile'])) : $query.= '`contact_mobile` = "' . $infos['mobile'] . '", '; else : $query.= '`contact_mobile` = NULL, '; 
        endif;
