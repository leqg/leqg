<?php
	...
	if (isset($_GET['contact'], $_GET['argumentaire'])) {
		...
		$query = $link->prepare('UPDATE `rappels` SET `rappel_statut` = 2 WHERE `contact_id` = :contact AND `argumentaire_id` = :argumentaire');
		...
		$query->bindParam(':objet', $argumentaire['argumentaire_nom']);
