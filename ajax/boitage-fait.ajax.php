...
if (in_array($immeuble, $afaire)) {
    ...
    if (!in_array($immeuble, $fait)) { $fait[] = $immeuble; 
    }
    ...
    foreach ($electeurs as $electeur) {
        $historique->ajout($electeur['id'], $_COOKIE['leqg-user'], 'boite', date('d/m/Y'), 'Domicile', $message, '');
    }
        
    $core->tpl_go_to('boite', array('action' => 'mission', 'mission' => $parcours['id']), true);
}
